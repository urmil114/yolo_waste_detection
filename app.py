...
# Classes to always force as recyclable (extended for pens/pencils etc.)
...
def handle_prediction(img_bgr, session, input_name, output_name, effnet_size):
    ...
    # ===== NEW FIX: Override hazardous stationery =====
    if label == "hazardous" and confidence >= 0.90:
        # If YOLO found nothing OR irrelevant, assume recyclable stationery
        if len(detected_classes) == 0 or all(obj not in NON_WASTE_CLASSES for obj in detected_classes):
            preds = [0.0, 0.0, 1.0]
            label = "recycleable"
            confidence = 1.0
